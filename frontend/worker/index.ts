...
function getPg(env: Env) {
  ...
  // Strip potential wrapping quotes from .dev.vars
  if ((dsn.startsWith('"') && dsn.endsWith('"')) || (dsn.startsWith("'") && dsn.endsWith("'"))) {
    dsn = dsn.slice(1, -1);
  }

  // Default to require, but allow disabling via DSN for local dev
  const ssl = dsn.includes('sslmode=disable') ? false : 'require';

  try {
    return postgres(dsn, { ssl });
  } catch (e: any) {
    console.error('getPg error', { dsnLen: dsn.length, dsnStart: dsn.slice(0, 20) + '...', error: e.message });
    throw e;
  }
}
...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      // Authenticated database-backed endpoints
      if (url.pathname === "/api/me") {
        ...
        if (request.method === 'GET') {
          ...
          return new Response(JSON.stringify({ ok: true, user: responseUser }), { headers: { 'content-type': 'application/json' } });
        }
      ...
      if (url.pathname === "/api/keys/gemini" || url.pathname === "/api/keys/gemini/") {
        const sess = await getSessionFromCookie(request, env);
        if (!sess) return new Response(JSON.stringify({ ok: false }), { status: 401, headers: { 'content-type': 'application/json' } });
        const user = await findUserByEmail(env, sess.email).catch(e => { console.error("gemini_key DB error", e); return null; });
        if (!user?.id) return new Response(JSON.stringify({ ok: false, error: 'user_not_found' }), { status: 404, headers: { 'content-type': 'application/json' } });
        const sql = getPg(env);
        if (request.method === 'GET') {
      ...
      if (url.pathname === '/api/stripe/products') {
        ...
        if (request.method === 'POST') {
          try {
            ...
            console.error('Stripe product creation error', e);
            return new Response(JSON.stringify({ ok: false, error: 'internal_error', message: e.message }), { status: 500, headers: { 'content-type': 'application/json' } });
          }
        }
        return new Response(null, { status: 405 });
      }
      ...
      if (url.pathname === '/api/stripe/sync-products' && request.method === 'POST') {
        try {
          ...
          console.error('Stripe sync-products error', e);
          return new Response(JSON.stringify({ ok: false, error: 'internal_error', message: e.message }), { status: 500, headers: { 'content-type': 'application/json' } });
        }
      }
      ...
      if (url.pathname === '/webhook/stripe/thin' && request.method === 'POST') {
        try {
          ...
          console.log('Stripe webhook (thin)', { type: event.type, id: event.id });

          // Handle specific events (thin processing - just log)
          switch (event.type) {
          ...
          console.error('Stripe webhook (thin) error', e);
      ...
      if (url.pathname === '/webhook/stripe/snapshot' && request.method === 'POST') {
        try {
          ...
          console.log('Stripe webhook (snapshot)', { type: event.type, id: event.id });

          // Handle specific events (snapshot processing - save to database)
          const sql = getPg(env);
          ...
          console.error('Stripe webhook (snapshot) error', e);
          return new Response(JSON.stringify({ error: e.message }), {
            status: 400,
            headers: { 'content-type': 'application/json' }
          });
      ...
      console.warn('Worker 404', { method: request.method, path: url.pathname });
      return new Response(null, { status: 404 });
    } catch (e: any) {
...
async function verifyStripeWebhook(body: string, signature: string, secret: string): Promise<any | null> {
  try {
...
async function handleGoogleCallback(request: Request, env: Env, url: URL): Promise<Response> {
  ...
  // Dev fallback: if origin is pointing to the Worker port (e.g. 18312), 
  // and we see we are on localhost, try to target the frontend port (18310).
  ...
  // Persist to database (optional in dev). Skip if config is missing.
...
async function upsertUser(env: Env, user: NewUser): Promise<void> {
  const sql = getPg(env);
...
async function upsertOAuthAccount(env: Env, acct: OAuthAccount): Promise<void> {
  const sql = getPg(env);
...
async function startInstagramOAuth(request: Request, env: Env, url: URL): Promise<Response> {
  // Modern Instagram API uses Facebook App credentials
  ...
  const origin = paramOrigin(url) || devOriginFromEnv(env) || effectiveOrigin(request, url); const state = await makeSignedState(env.SESSION_SECRET, { origin });
  const redirectUri = `${origin}/api/auth/instagram/callback`;
  // Use Facebook OAuth endpoint (Instagram is now integrated into Facebook platform)
  ...
  const headers = new Headers({ Location: authorize.toString() });
  const secure = isHttps(request, url);
  // Use root path so cookie is available on callback (important for proxy setups)
...
async function handleIGGraphCallback(request: Request, env: Env, url: URL): Promise<Response> {
  ...
  console.log('Facebook Pages found', { count: list.length, pages: list.map(p => ({ id: p.id, name: p.name })) });
  ...
  for (const p of list) {
    ...
    if (!igRes.ok) {
      console.log(`Page ${p.name} (${p.id}): No IG account - ${igRes.status}`);
      continue;
    }
    const igJson = await igRes.json() as any;
    const ig = igJson?.instagram_business_account?.id as string | undefined;
    if (!ig) {
      console.log(`Page ${p.name} (${p.id}): No instagram_business_account field`);
      continue;
    }
    console.log(`Page ${p.name} (${p.id}): Found IG account ${ig}`);
    // Fetch IG username
  ...
  console.log('Instagram Graph callback complete', {
    savedAny,
    targetOrigin,
    msg
  });
  ...
  const html = `<!doctype html><html><body><script>
    (function(){
      try {
        ...
        console.log('Sending postMessage', { data: data, targetOrigin: targetOrigin });
        if (window.opener && typeof window.opener.postMessage === 'function') {

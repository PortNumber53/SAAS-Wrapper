...
import type { OrderItemsRecord } from "@/vendor/xata";
...
const webhookSecret =
  ...
  "whsec_130754854f959745d88bf2b77cf6a1a8db1ad0390978a128b0620a87f92a2a56";
  ...
  try {
    ...
    try {
      ...
      console.error(
        ...
        JSON.stringify(
          handlerError,
          Object.getOwnPropertyNames(handlerError),
          2
        )
      );
    }
...
async function handleCheckoutSessionCompleted(
  ...
  try {
    ...
    const orderItems = await xata.db.order_items
      .filter({ order_id: order.id })
      .getAll();

    if (!orderItems?.length) {
      console.error(`No order items found for order: ${order.id}`);
      return;
    }

    await Promise.all(
      orderItems.map(async (item: OrderItemsRecord) => {
        if (!item?.quantity || !item.product_id) {
          console.error(`Invalid order item data: ${JSON.stringify(item)}`);
          return;
        }

        const product = await xata.db.products.read(item.product_id);
        if (!product) {
        ...
        await xata.db.products.update(product.id, {
          inventory_count: Math.max(
            0,
            (product.inventory_count || 0) - item.quantity
          ),
        });
      })
    );

    // Update user's Stripe customer info if needed
...
async function handleSubscriptionCreated(subscription: Stripe.Subscription) {
  try {
    ...
    const existingSub = await xata.db.subscriptions
      .filter({
...
async function handleSubscriptionUpdated(subscription: Stripe.Subscription) {
  try {
    ...
    if (!existingSub) {
      ...
      console.log(
        `Created new subscription from update event ${subscription.id}`
      );
      return;
    }
    ...
    console.log(
      "Prepared update data existingSubscription:",
      JSON.stringify(existingSub, null, 2)
    );

    await xata.db.subscriptions.update(existingSub.id, subscriptionData);
    console.log(
      `Updated subscription ${subscription.id} with status: ${subscription.status}`
    );
  } catch (error) {
...
async function handleSubscriptionCanceled(subscription: Stripe.Subscription) {
  const sub = await xata.db.subscriptions
    .filter({
      stripeSubscriptionId: subscription.id,
    })
    .getFirst();
...
async function handleInvoiceEvent(eventType: string, invoice: Stripe.Invoice) {
  ...
  try {
    const existingInvoice = await xata.db.invoices
      .filter({
    ...
    if (!existingInvoice) {
      ...
      console.log(
        "Prepared update data existingInvoice:",
...
async function findUserByEmailOrStripeCustomerId(customer: Stripe.Customer) {
  console.log(
    `Searching for user: Email=${customer.email}, Stripe Customer ID=${customer.id}`
  );

  // First, try to find by Stripe Customer ID
  let existingUser = await xata.db.nextauth_users
    .filter({
  ...
  if (!existingUser && customer.email) {
    existingUser = await xata.db.nextauth_users
      .filter({
    ...
    if (existingUser) {
      ...
      console.log(
        "Prepared update data existingUser:",
  ...
  try {
    ...
    if (existingUser) {
      ...
      try {
        ...
        const updatedUser = await xata.db.nextauth_users.update(
          existingUser.id,
          updateData
        );
        console.log(
          `Successfully updated user ${existingUser.id}:`,
      ...
      console.warn(
        `No user found for Stripe Customer ID: ${customer.id} or email: ${customer.email}`
      );
    }
  } catch (error) {

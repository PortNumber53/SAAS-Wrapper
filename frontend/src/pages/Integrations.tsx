...
const SUPPORTED = [
  ...
  { key: 'iggraph', name: 'Instagram' },  // Consolidated - uses Facebook Graph API
]
...
export default function IntegrationsPage() {
  ...
  useEffect(() => {
    const onMsg = (ev: MessageEvent) => {
      ...
      if ((ev.data.type === 'oauth:instagram' || ev.data.type === 'oauth:iggraph' || ev.data.type === 'oauth:google') && ev.data.data?.ok) {
  ...
  const connect = async (provider: string) => {
    const w = 600, h = 800  // Larger size to fit Facebook OAuth content
    ...
    if (provider === 'iggraph') {
      const start = new URL('/api/auth/iggraph/start', window.location.origin)
      start.searchParams.set('origin', window.location.origin)
      window.open(start.toString(), '_blank', `popup=yes,width=${w},height=${h},top=${y},left=${x}`)
      return
    }
                    ...
                    <div style={{ display: 'flex', gap: 8 }}>
                      <button className='btn' onClick={async () => {
                        ...
                        if (!res.ok) {
                          ...
                          try {
                            const j = JSON.parse(t); if (j?.error === 'reauthorization_required' || j?.error === 'ig_account_not_linked') {
                              window.open('/api/auth/iggraph/start', '_blank', 'popup=yes,width=480,height=640');
                            } else { toast.show(t, 'error') }
                          } catch { toast.show(t, 'error') }
                        }
